...
def augment_patches(hr_patch, lr_patch):
    ...
    # Versión original (sin transformación)
...
def prepare_data_rgb(base_path, save_path, patch_size_hr=128, patch_size_lr=32,
                     random_number=10, scale=4, start_index=1, end_index=200,
                     batch_size=500):
    ...
    print(f"Procesando {num_images} imágenes desde el índice {start_index} hasta {end_index}")
    ...
    for i, file_name in enumerate(file_names):
        ...
        for _ in range(random_number):
            ...
            # Guardar si se alcanza batch_size
    ...
    if len(hr_data_list) > 0:
        save_batch_to_h5(hr_data_list, lr_data_list, save_path,
                         idx, patch_size_hr, patch_size_lr)
        print(f"[INFO] Guardado el lote final con {len(hr_data_list)} elementos.")


def prepare_data_with_noise(base_path, save_path, patch_size_hr=128, patch_size_lr=32,
                            random_number=10, scale=4, snr=20, start_index=1,
                            end_index=200, batch_size=500):
    ...
    for i, file_name in enumerate(file_names):
        ...
        h, w, _ = hr_img.shape
        if h < patch_size_hr or w < patch_size_hr:
            print(f"La imagen {file_name} es demasiado pequeña para el parche HR.")
            continue
        ...
        for _ in range(random_number):
            ...
            # Convertir a float y normalizar antes de añadir ruido
            hr_patch = hr_patch.astype(np.float32) / 255.0
            lr_patch = lr_patch.astype(np.float32) / 255.0

            # Añadir ruido SNR a LR
            lr_patch_noisy = add_noise_with_snr(lr_patch, snr)

            # Data augmentation
            # Observa que hr_patch NO cambia, el ruido se aplica solo a LR
    ...
    if len(hr_data_list) > 0:
        save_batch_to_h5(hr_data_list, lr_data_list, save_path,
                         idx, patch_size_hr, patch_size_lr)
        print(f"Guardado el lote final con {len(hr_data_list)} elementos.")
...
if __name__ == "__main__":
    ...
    data_train_dir_normal = os.path.join(root_dir, "Data", "Train", "Normal")
    data_train_dir_noise = os.path.join(root_dir, "Data", "Train", "Noise")
    os.makedirs(data_train_dir_normal, exist_ok=True)
    os.makedirs(data_train_dir_noise, exist_ok=True)

    save_path_train_normal = os.path.join(data_train_dir_normal, "train_data_normal.h5")
    save_path_train_noise = os.path.join(data_train_dir_noise, "train_data_noise.h5")

    # Datos sin ruido (normal), con data augmentation
    prepare_data_rgb(
        base_path=base_path,
    ...
    # Datos con ruido, también con data augmentation
    prepare_data_with_noise(
        base_path=base_path,
